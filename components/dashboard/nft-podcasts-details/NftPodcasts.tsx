...
import { youtubeParser } from '../../../utils/formatter';
import { Loader } from '../../Loader';
import { NewsEmail } from '../news/NewsEmail';
// import { CustomForm } from '../news/MailForm';
...
interface NftPodcastsProps {
  detail: INettyNews;
}

const NftPodcasts: React.FC<NftPodcastsProps> = ({ detail }) => {
  ...
  const youtubeLink = youtubeParser(detail?.content?.rendered as string);
    ...
    <div>
      {detailLoading ? (
        ...
        <div>
          ...
          <div className='flex gap-3 items-center pl-8 pt-[50px] md:pt-[13px] max-w-[1300px] 2xl:mx-auto' onClick={handleBack}>
            <LeftIcon fill='#006FFF' className='w-[17px] h-[16px]' />
            <span className='text-lg'>Back to </span>
            <span className='text-lg font-bold'>Podcasts</span>
          </div>
          <div className='pt-6 sm:h-auto'>
            <div className=' px-8 '>
              <h1 className='text-9 text-[34px] lg:text-[40px] text-primary font-bold pt max-w-[1300px] 2xl:mx-auto'>{detail?.title.rendered}</h1>
              <div className='flex pt-3 justify-between max-w-[1300px] 2xl:mx-auto'>
            ...
            <div>
              <div className='h-[400px] sm:h-[628px] lg:h-auto px-8 flex-col lg:flex-row lg:pb-[90px] flex justify-between gap-3 pt-[21px] relative max-w-[1300px] 2xl:mx-auto'>
                <div className='flex-row flex lg:flex-col gap-3 h-full items-end lg:h-auto lg:items-start p-3'>
                ...
                <div className='w-full dark-primary h-[320px] px-7 sm:h-[549px] rounded  sm:min-h-max absolute right-0 lg:max-w-[94%] xl:max-w-[95%]'>
                  {!youtubeLink ? (
                    <Loader />
                  ) : (
                    <ReactPlayer
                      url={youtubeLink}
                      playIcon={
                        <div className='h-[66px] w-[66px] bg-white rounded py-5 px-6'>
                          <FontAwesomeIcon icon={faPlay} className='text-primary w-[20px] h-[23px]' />
                        </div>
                      }
                      light={true}
                      width='100%'
            ...
            <div className='bg-light-blue-grey w-full border-t-2 pb-20 sm:pb-8 '>
              <div className='flex flex-col pl-8 pr-8 xl:pr-0 gap-10 lg:mt-[320px] lg:pl-20 2xl:pl-15 md:pr-8 xl:flex-row lg:flex bg-light-blue-grey xl:max-w-[1130px] 2xl:max-w-[1300px] 2xl:mx-auto pb-2 podcasts_detail'>
                <div className='flex flex-col pb-1 lg:pb-44'>
                  <span className='text-base leading-7 news_article' dangerouslySetInnerHTML={{ __html: detail?.content.rendered }}></span>
                </div>
                <NewsEmail />
